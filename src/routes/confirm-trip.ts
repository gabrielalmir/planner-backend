...
export async function confirmTrip(app: FastifyInstance) {
    app.withTypeProvider<ZodTypeProvider>().get('/trips/:tripId/confirm', { schema: ConfirmTripSchema }, async (request, reply) => {
        ...
        await Promise.all(trip.participants.map(async participant => {
            ...
            const message = await mail.sendMail({
                ...
                subject: `Planner - Confirme sua presença na viagem para ${trip.destination} em ${formattedStartDate}`,
                html: `
